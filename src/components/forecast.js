...
const Forecast = ({ dailyWeather, weatherMapping }) => {
    ...
    const forecastData = dailyWeather?.temperature_2m_max.slice(0, 4).map((maxTemp, index) => {
        const forecastDate = new Date(today);
        forecastDate.setDate(todayDate + index);

        const dayLabel = index === 0 ? "Today" : index === 1 ? "Tomorrow" : forecastDate.toLocaleDateString("en-US", { weekday: "long" });
